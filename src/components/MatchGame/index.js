...
class MatchGame extends Component {
  ...
  onClickImage = event => {
    const {randomGeneratingImage} = this.props
    if (event.target.src === randomGeneratingImage) {
      this.setState(prevState => ({points: prevState.points + 1}))
    }
    console.log(randomGeneratingImage)
  }

  render() {
    const {activeTabId, stopWatch, points} = this.state
    const randomValue = Math.floor(Math.random() * imagesList.length - 1)
    console.log(randomValue)

    const randomGeneratingImage = imagesList[randomValue].imageUrl
    const filteredImages = this.getActiveTabImages()

    return (
      <div className="match-game-bg">
        <NavBar stopWatch={stopWatch} points={points} />
        <div>
          <img
            src={randomGeneratingImage}
            alt="random-img"
